      ...
      <div style={{ maxWidth: 1240, margin: '0 auto', padding: '0 24px' }}>
            ...
            {['ALL', 'WATERMARKED', 'VERIFIED', 'UPLOADED'].map(st => (
              <button
                ...
                style={{
                  ...
                  color: statusFilter === st ? '#00e5ff' : '#6b7a8d',
        ...
        {loading ? (
          <div style={{ textAlign: 'center', padding: '64px 0' }}>
            ...
            <p style={{ color: '#6b7a8d', fontSize: '0.9rem' }}>Loading your CAD projects...</p>
          </div>
        ) : filtered.length === 0 ? (
          <GlassCard style={{ padding: '64px 24px', textAlign: 'center' }}>
            ...
            <p style={{ color: '#6b7a8d', fontSize: '0.85rem', maxWidth: 460, margin: '0 auto 24px' }}>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(360px, 1fr))', gap: 20 }}>
            {filtered.map((proj, idx) => {
              ...
              }[proj.status?.toLowerCase()] || { bg: 'rgba(255,255,255,0.05)', text: '#8892a4', border: 'rgba(255,255,255,0.1)' }
                  ...
                  <GlassCard style={{ padding: 24, height: '100%', boxSizing: 'border-box', display: 'flex', flexDirection: 'column', justifyContent: 'space-between' }}>
                    <div>
                      ...
                      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 14 }}>
                        ...
                        <span style={{ fontSize: '0.72rem', color: '#4a5568', display: 'flex', alignItems: 'center', gap: 4 }}>
                          <Calendar size={12} />
                      ...
                      <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 16 }}>
                        ...
                        {proj.project_id && (
                          <button
                            ...
                            style={{ background: 'none', border: 'none', color: copiedId === proj.project_id ? '#22c55e' : '#4a5568', cursor: 'pointer', padding: 3 }}
                      ...
                      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: 8, padding: '10px 12px', borderRadius: 10, background: 'rgba(255,255,255,0.02)', border: '1px solid rgba(255,255,255,0.04)', marginBottom: 18 }}>
                        <div>
                          <div style={{ fontSize: '0.65rem', color: '#4a5568', textTransform: 'uppercase' }}>Format</div>
                          <div style={{ fontSize: '0.8rem', fontWeight: 700, color: '#f0f4ff', textTransform: 'uppercase' }}>{proj.file_format || 'STL'}</div>
                        </div>
                        <div>
                          <div style={{ fontSize: '0.65rem', color: '#4a5568', textTransform: 'uppercase' }}>Vertices</div>
                          <div style={{ fontSize: '0.8rem', fontWeight: 700, color: '#f0f4ff' }}>{proj.vertex_count?.toLocaleString() || '—'}</div>
                        </div>
                        <div>
                          <div style={{ fontSize: '0.65rem', color: '#4a5568', textTransform: 'uppercase' }}>Integrity</div>
                      ...
                      <Link to="/viewer" style={{ textDecoration: 'none' }}>
                        <button
                          style={{
                            padding: '9px 12px', borderRadius: 10,
                            background: 'rgba(255,255,255,0.04)', border: '1px solid rgba(255,255,255,0.08)',
                            color: '#8892a4', fontSize: '0.8rem', fontWeight: 600, cursor: 'pointer',
                            display: 'flex', alignItems: 'center', gap: 6,
                          }}
                        >
                          <Eye size={14} /> 3D View
